...
#define MAX_QUEUE_SIZE (15 * 1024 * 1024) /*15MB*/
...
{				   //使用fifo保存MyAVPacketList，所以删除了旧版的next指针
	AVPacket *pkt; //编码包
	int serial;	   //播放序列
} MyAVPacketList;
...
{
	AVFifoBuffer *pkt_list; //MyAVPacketList的fifo
	int nb_packets;			//fifo中元素数量
	int size;				//队列所有元素的数据大小总和
	int64_t duration;		//队列所有元素的数据播放持续时间
	int abort_request;		//退出请求标志
	int serial;				//当队列中加入一个flush_pkt后，后续节点的serial会比之前大1，用来区别不同播放序列的packet
	SDL_mutex *mutex;
	SDL_cond *cond;
} PacketQueue;
...
#define SAMPLE_QUEUE_SIZE 9		   //音频帧缓存数量
...
typedef struct AudioParams
{
	...
	int bytes_per_sec;		 //一秒字节数
} AudioParams;

typedef struct Clock
{
	double pts; /* clock base */																		   //时钟基础, 当前帧(待播放)显示时间戳，播放后，当前帧变成上一帧
	double pts_drift; /* clock base minus time at which we updated the clock */							   //当前pts与当前系统时钟的差值
	...
	int paused;																							   //暂停状态
	int *queue_serial; /* pointer to the current packet queue serial, used for obsolete clock detection */ //指向packet的serial
} Clock;
...
typedef struct Frame
{
	...
	double duration; /* estimated duration of the frame */			//持续时间
	int64_t pos; /* byte position of the frame in the input file */ //文件位置
	int width;														//图像宽度
	int height;														//图像高度
	int format;														//数据格式
	AVRational sar;													//图像宽高比
	int uploaded;													//记录是否已经渲染本帧
	int flip_v;														//旋转标记
} Frame;
...
typedef struct FrameQueue
{
	...
	SDL_mutex *mutex;
	SDL_cond *cond;
	PacketQueue *pktq; //编码包缓冲队列
} FrameQueue;

enum
{
	AV_SYNC_AUDIO_MASTER,
	/* default choice */  //音频为基准
	AV_SYNC_VIDEO_MASTER, //视频为基准
	AV_SYNC_EXTERNAL_CLOCK,
	/* synchronize to an external clock */ //外部时钟为基准
};
...
typedef struct Decoder
{
	...
	AVRational start_pts_tb; //开始时基
	int64_t next_pts;		 //推算下一帧显示时间戳
	AVRational next_pts_tb;	 //推送下一帧时基
	SDL_Thread *decoder_tid; //解码线程
} Decoder;

typedef struct VideoState
{
	SDL_Thread *read_tid;	//读数据线程
	AVInputFormat *iformat; //解复用器
	int abort_request;		//退出播放
	int force_refresh;		//刷新标记
	int paused;				//暂停标记
	int last_paused;		//暂存状态
	int queue_attachments_req;
	int seek_req;	  //seek请求
	int seek_flags;	  //seek参数
	...
	double audio_clock;		//当前音频pts+duration
	int audio_clock_serial; //音频序列
	double audio_diff_cum;	/* used for AV difference average computation */
	double audio_diff_avg_coef;
	double audio_diff_threshold;
	int audio_diff_avg_count;
	AVStream *audio_st;							//音频流
	PacketQueue audioq;							//音频编码包队列
	...
	int audio_buf_index; /* in bytes */			//更新拷贝位置
	int audio_write_buf_size;					//当前音频帧中尚未拷入SDL音频缓冲区的数据量
	int audio_volume;							//音量
	int muted;									//静音
	...
	int frame_drops_early;		  //丢弃视频编码包计数
	int frame_drops_late;		  //丢弃视频解码包计数
	...
	SDL_Texture *vid_texture; //视频渲染纹理
	...
	PacketQueue videoq;					//视频编码包队列
	double max_frame_duration;			// maximum duration of a frame - above this, we consider the jump a timestamp discontinuity //一帧最大间隔
	struct SwsContext *img_convert_ctx; //图像变换上下文
	struct SwsContext *sub_convert_ctx;
	int eof; //读取结束标记

	char *filename;					//输入名
	int width, height, xleft, ytop; //宽、高、x、y
	int step;						//步进播放
...
{
	double time = av_gettime_relative() / 1000000.0; //获取当前时间(s)
...
int main(int argc, char **argv)
{
	...
	else
	{
		...
		//当周期大小为NPOT时，尝试通过强制缓冲区大小来解决偶然的ALSA缓冲区下溢问题?
